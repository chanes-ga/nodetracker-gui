...
<?php
	...
	switch ($updateRecord){
		...
		if ($cleartext=="on"){
			...
			$public=cryptText(1,$public,$cipher,$mode,$HTTP_SESSION_VARS["key"]);
                ...
                $sql="update Devices set active=$active,icmpscan=$icmpscan,groupid=$groupid,snmpver=$snmpver,IPAutoFill=0,ifDescShortOID=$ifDescShortOID,
	...
	$sql="select active,icmpscan,groupid,snmpver,ifDescShortOID, Devices.Description as Device, OSGuess, tcpsequence, 
	...
	if (mysql_num_rows($q)>0){
		...
		$editBlock=getEditBlock($nodeID,$r->Device,$r->RunNMAP,$r->public,$r->PrimaryIP,$r->type,$r->ifDescShortOID,$r->snmpver,$r->groupid,$r->icmpscan,$r->active);
