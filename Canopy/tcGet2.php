...
function buildCMDs($direction,$if,$speed,$pid,$classid,$ipblock,$mask,$subClassCount)
{
	...
	if($subClassCount==0){
		...
		$filter=getIPTableCMD($direction,$if,$ipblock,$mask,"ip","",getClassID($classid,0),1);
	}else{
...
function getSubClassFilters($direction,$if,$pid,$parentclassid,$subclassid,$ipblock,$mask)
{
	...
	$filter="";
		...
		$filter=$filter.getIPTableCMD($direction,$if,$ipblock,$mask,$protocol,$portlist,getClassID($parentclassid,$subclassid),$pdirection);
                $r=mysql_fetch_object($q);
        }
...
function getIPTableCMD($direction,$if, $ipblock,$mask,$protocol,$portlist,$cbqclass,$pdirection){
	$ipinfo=$ipblock."/".$mask;
        if ($direction==0){
        	#download so
                $ipinfo=" -d ".$ipinfo;
