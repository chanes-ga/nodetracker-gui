...
# 07/28/05:v1.1.0 iptable configs modified to employ user-defined chains for improved scalability and performance
...
# 08/12/05:v1.3.0 iptable chains now go 3 levels deep instead of 2
# 			Level 1: POSTROUTING chain matches layer 3
#			Level 2: user-chain matches layer 4 & layer 7
#			Level 3: user-chain specifies actions to take on packet: classify, log, and accept
# 08/18/05:v1.3.1 support for port ranges delimited by : (had to remove -m mulitport for these cases)
##########################################################################################################################################
...
function getSubClasses($direction,$pid,$parentclassid,$if,$speed,$ipblock,$mask)
{
	...
	$sql="select isdefault,ClassID,BWPercentage,BWCeiling,BWPercentageU,BWCeilingU from CanopyQOSClasses 
		where pid=$pid order by isdefault,ClassID";
	$q=mysql_query($sql);
	...
	while($r){
		...
		$class=$class.getClassChain($parentclassid,$r->ClassID,$if);
...
function getClassChain($parentclassid,$classid,$if){
	$classchain="chain".($parentclassid+$classid).$if;
	$cmd="\niptables -t mangle -X $classchain\niptables -t mangle -N $classchain\n";

	$cmd=$cmd."iptables -t mangle -A $classchain -p ip -j CLASSIFY --set-class ".getClassID($parentclassid,$classid)."\n";
	$cmd=$cmd."iptables -t mangle -A $classchain -p ip -j ULOG --ulog-nlgroup 1 --ulog-cprange 20 --ulog-prefix ".($parentclassid+$classid)."\n";
	$cmd=$cmd."iptables -t mangle -A $classchain -p ip -j ACCEPT\n\n";
	return $cmd;
}
...
function getIPTableCMD($direction,$if, $ipblock,$mask,$protocol,$portlist,$parentid,$childid,$pdirection,$isbaseclass,$layer7,$isdefault){
	...
	$classchain="chain".($childid+$parentid)."$if";
	$ipinfo=$ipblock."/".$mask;
        if ($direction==0){
        	#download so
                $ipinfo=" -d ".$ipinfo;
        ...
        if($portlist<>''){
		$extension="-m multiport";
		if (strpos($portlist,":")==true){
			#port range was specified so don't use -m multiport
			$extension="";
		}
        	$portlist=" $extension $portdirection $portlist  ";
        }else{
        	$portlist="";
	...
	if ($isbaseclass==0){
		$cmd="iptables -t mangle $insertpos $mychain -p $protocol $portlist $layer7 -j $classchain\n";
	}else{
